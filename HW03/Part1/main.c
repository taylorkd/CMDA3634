...
if(rank==1){  //Encrypt the message with rank 0's ElGamal cyrptographic system
...
if(rank==1){
for(unsigned int i=0;i<Nmessages;i++){
MPI_Send(&message[i],1,MPI_UNSIGNED ,0,0,MPI_COMM_WORLD);
MPI_Send(&a[i],1,MPI_UNSIGNED,0,0,MPI_COMM_WORLD);
}
}
...
for(unsigned int i=0;i<Nmessages;i++)
{
 MPI_Recv(&message[i],1,MPI_UNSIGNED,1,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
 MPI_Recv(&a[i],1,MPI_UNSIGNED,1,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
}
}
if(rank==0){
